...
// Cube color when selected: Green.
static const float kCubeSelectedColors[NUM_CUBE_COLORS] = {
    // front, green
    0.0f, 0.5273f, 0.2656f, 1.0f,
    0.0f, 0.5273f, 0.2656f, 1.0f,
    0.0f, 0.5273f, 0.2656f, 1.0f,
...
@implementation TreasureHuntRenderer {
    ...
    GLfloat _cube_vertices[NUM_CUBES][NUM_CUBE_VERTICES];
    GLfloat _cube_position[NUM_CUBES][3];
    GLfloat _cube_colors[NUM_CUBES][NUM_CUBE_COLORS];
    GLfloat _cube_found_colors[NUM_CUBES][NUM_CUBE_COLORS];
    GLfloat _cube_selected_colors[NUM_CUBES][NUM_CUBE_COLORS];
    
    GLuint _cube_program[NUM_CUBES];
    GLint _cube_vertex_attrib[NUM_CUBES];
    GLint _cube_position_uniform[NUM_CUBES];
    GLint _cube_mvp_matrix[NUM_CUBES];
    GLuint _cube_vertex_buffer[NUM_CUBES];
    GLint _cube_color_attrib[NUM_CUBES];
    GLuint _cube_color_buffer[NUM_CUBES];
    GLuint _cube_found_color_buffer[NUM_CUBES];
    GLuint _cube_selected_color_buffer[NUM_CUBES];
    ...
    GCSCardboardAudioEngine *_cardboard_audio_engine;
    int _sound_object_id;
    bool _is_cube_focused[NUM_CUBES];
    bool _is_cube_selected[NUM_CUBES];
}
...
- (void)cardboardView:(GCSCardboardView *)cardboardView
    ...
    for(int index=0; index<NUM_CUBES; index++){
        _cube_program[index] = glCreateProgram();
        NSAssert(_cube_program != 0, @"Failed to create program");
        glAttachShader(_cube_program[index], vertex_shader);
        glAttachShader(_cube_program[index], fragment_shader);
        
        // Link the shader program.
        glLinkProgram(_cube_program[index]);
        NSAssert(checkProgramLinkStatus(_cube_program[index]), @"Failed to link _cube_program");
        
        // Get the location of our attributes so we can bind data to them later.
        _cube_vertex_attrib[index] = glGetAttribLocation(_cube_program[index], "aVertex");
        NSAssert(_cube_vertex_attrib[index] != -1, @"glGetAttribLocation failed for aVertex");
        _cube_color_attrib[index] = glGetAttribLocation(_cube_program[index], "aColor");
        NSAssert(_cube_color_attrib[index] != -1, @"glGetAttribLocation failed for aColor");
        
        // After linking, fetch references to the uniforms in our shader.
        _cube_mvp_matrix[index] = glGetUniformLocation(_cube_program[index], "uMVP");
        _cube_position_uniform[index] = glGetUniformLocation(_cube_program[index], "uPosition");
        NSAssert(_cube_mvp_matrix[index] != -1 && _cube_position_uniform[index] != -1,
                 @"Error fetching uniform values for shader.");
        // Initialize the vertex data for the cube mesh.
        for (int i = 0; i < NUM_CUBE_VERTICES; ++i) {
            _cube_vertices[index][i] = (GLfloat)(kCubeVertices[i] * kCubeSize);
        }
        glGenBuffers(1, &_cube_vertex_buffer[index]);
        NSAssert(_cube_vertex_buffer[index] != 0, @"glGenBuffers failed for vertex buffer");
        glBindBuffer(GL_ARRAY_BUFFER, _cube_vertex_buffer[index]);
        glBufferData(GL_ARRAY_BUFFER, sizeof(_cube_vertices[index]), _cube_vertices[index], GL_STATIC_DRAW);
        
        // Initialize the color data for the cube mesh.
        for (int i = 0; i < NUM_CUBE_COLORS; ++i) {
            _cube_colors[index][i] = (GLfloat)(kCubeColors[i] * kCubeSize);
        }
        glGenBuffers(1, &_cube_color_buffer[index]);
        NSAssert(_cube_color_buffer[index] != 0, @"glGenBuffers failed for color buffer");
        glBindBuffer(GL_ARRAY_BUFFER, _cube_color_buffer[index]);
        glBufferData(GL_ARRAY_BUFFER, sizeof(_cube_colors[index]), _cube_colors[index], GL_STATIC_DRAW);
        
        // Initialize the found color data for the cube mesh.
        for (int i = 0; i < NUM_CUBE_COLORS; ++i) {
            _cube_found_colors[index][i] = (GLfloat)(kCubeFoundColors[i] * kCubeSize);
        }
        glGenBuffers(1, &_cube_found_color_buffer[index]);
        NSAssert(_cube_found_color_buffer[index] != 0, @"glGenBuffers failed for color buffer");
        glBindBuffer(GL_ARRAY_BUFFER, _cube_found_color_buffer[index]);
        glBufferData(GL_ARRAY_BUFFER, sizeof(_cube_found_colors[index]), _cube_found_colors[index], GL_STATIC_DRAW);
        
        // Initialize the selected color data for the cube mesh.
        for (int i = 0; i < NUM_CUBE_COLORS; ++i) {
            _cube_selected_colors[index][i] = (GLfloat)(kCubeSelectedColors[i] * kCubeSize);
        }
        glGenBuffers(1, &_cube_selected_color_buffer[index]);
        NSAssert(_cube_selected_color_buffer[index] != 0, @"glGenBuffers failed for color buffer");
        glBindBuffer(GL_ARRAY_BUFFER, _cube_selected_color_buffer[index]);
        glBufferData(GL_ARRAY_BUFFER, sizeof(_cube_selected_colors[index]), _cube_selected_colors[index], GL_STATIC_DRAW);
    ...
    [[GCSCardboardAudioEngine alloc] initWithRenderingMode:kRenderingModeBinauralHighQuality];
    ...
    _sound_object_id = [_cardboard_audio_engine createSoundObject:kSampleFilename];
    
    for(int index = 0; index<NUM_CUBES; index++){
        [self spawnCube:index];
    }
}
...
- (void)cardboardView:(GCSCardboardView *)cardboardView
    ...
    for(int index = 0; index<NUM_CUBES; index++){
        ...
        GLKVector3Make(_cube_position[index][0], _cube_position[index][1], _cube_position[index][2]);
        _is_cube_focused[index] = [self isLookingAtObject:&head_rotation sourcePosition:&source_cube_position];
    }
...
- (void)renderWithModelViewProjectionMatrix:(const float *)model_view_matrix {
    ...
    for(int index = 0; index<NUM_CUBES; index++){
        glUseProgram(_cube_program[index]);
        
        // Set the uniform values that will be used by our shader.
        glUniform3fv(_cube_position_uniform[index], 1, _cube_position[index]);
        
        // Set the uniform matrix values that will be used by our shader.
        glUniformMatrix4fv(_cube_mvp_matrix[index], 1, false, model_view_matrix);
        
        // Set the cube colors.
        if (_is_cube_focused[index]) {
            glBindBuffer(GL_ARRAY_BUFFER, _cube_found_color_buffer[index]);
        } else {
            if(_is_cube_selected[index]){
                glBindBuffer(GL_ARRAY_BUFFER, _cube_selected_color_buffer[index]);
            }else{
                glBindBuffer(GL_ARRAY_BUFFER, _cube_color_buffer[index]);
            }
        }
        glVertexAttribPointer(_cube_color_attrib[index], 4, GL_FLOAT, GL_FALSE, sizeof(float) * 4, 0);
        glEnableVertexAttribArray(_cube_color_attrib[index]);
        
        // Draw our polygons.
        glBindBuffer(GL_ARRAY_BUFFER, _cube_vertex_buffer[index]);
        glVertexAttribPointer(_cube_vertex_attrib[index], 3, GL_FLOAT, GL_FALSE,
                              sizeof(float) * 3, 0);
        glEnableVertexAttribArray(_cube_vertex_attrib[index]);
        glDrawArrays(GL_TRIANGLES, 0, NUM_CUBE_VERTICES / 3);
        glDisableVertexAttribArray(_cube_vertex_attrib[index]);
    }
...
- (void)spawnCube:(int)index{
    ...
    //[self setRandomCubePosition:kMinCubeDistance maxLimit:kMaxCubeDistance cubeIndex:index];
    if(index == TARGET_INDEX){
        [self resetCubes];
    }else{
        _is_cube_selected[index]=true;
    }
    [_cardboard_audio_engine setSoundObjectPosition:_sound_object_id
                                                  x:_cube_position[index][0]
                                                  y:_cube_position[index][1]
                                                  z:_cube_position[index][2]];
    [_cardboard_audio_engine playSound:_sound_object_id loopingEnabled:true];
}

- (void)resetCubes{
    for(int index=0; index<NUM_CUBES; index++){
        [self setRandomCubePosition:kMinCubeDistance maxLimit:kMaxCubeDistance cubeIndex:index];
        _is_cube_selected[index] = false;
    }
}

// Sets a new position for the cube.
- (void)setRandomCubePosition:(float)min maxLimit:(float)max cubeIndex:(int)index{
    ...
    _cube_position[index][0] = -cos(elevation) * sin(azimuth) * distance;
    _cube_position[index][1] = sin(elevation) * distance;
    _cube_position[index][2] = -cos(elevation) * cos(azimuth) * distance;
}
